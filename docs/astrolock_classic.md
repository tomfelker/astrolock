...
This is now working as **AstroLock Seeker** (see the section above): a separate, file-driven
pipeline that captures video, detects the target, and closes the tracking loop in sky-direction
space, handing off from the guide camera to the main camera as the target settles in. The auditory
"Sidewinder" lock-on feedback is still on the wishlist.

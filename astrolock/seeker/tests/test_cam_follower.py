...
def test_cam_to_follower():
    ...
    ref, frame = res
    assert ref.index == 19
    assert ref.ser_path.endswith('.ser')           # segment identity (ident + '.ser', virtual)
    ...
    # v3: records are binary structs INSIDE the segment; the head file announces segments.
    seg = f.segs[-1]
    assert seg.meta['width'] == 320 and seg.committed() == 20
    assert seg.finalized() and seg.stream_ended()
    r = seg.record(19)
